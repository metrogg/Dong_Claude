...
# Run daemon manually
uv run dong-core                        # foreground; Ctrl+C to stop
DONG_PORT=8000 uv run dong-core        # override port
...
This is a **dual-process** local AI agent system. `dong-core` is a persistent daemon; `dong` and `dong-tui` are clients that connect to it over a Unix domain socket.

```
dong-core (daemon)
  └─ listens on 127.0.0.1:7437 (TCP)
       ↑ JSON-RPC 2.0 NDJSON
dong (CLI)   dong-tui (TUI, S2+)
```

**`dong-tui` is the primary frontend.** All user-facing work on task management, observability, and interaction should be designed for and validated in the TUI first. The `dong` CLI exists only for quick scripted testing and debugging — it is not a product surface. When implementing features that touch the user interface, invest in the TUI layout, event rendering, and keyboard interactions. Do not shortcut TUI work by pointing to the CLI as an alternative.

### Protocol layer (`src/dong_claude/core/bus/`)
...
### Transport layer (`src/dong_claude/core/transport/`)
...
### Config (`src/dong_claude/core/config.py`)

Four-tier priority: **built-in defaults → `~/.dong/config.toml` → `.env` → env vars**.
...
Relevant env vars: `DONG_CONFIG`, `DONG_HOST`, `DONG_PORT`, `DONG_LOG_LEVEL`, `DONG_LOG_FILE`, `DONG_LOG_FORMAT`.

### Daemon entry (`src/dong_claude/core/app.py`)
...
Integration tests in `tests/conftest.py` spawn a real daemon subprocess using a random free port (via `free_port` fixture). The fixture finds a free port, releases it, passes it to the daemon via `DONG_PORT`, then polls `asyncio.open_connection` until the daemon is ready.

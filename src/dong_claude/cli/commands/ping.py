...
import dong_claude
from dong_claude.core.bus.commands import PongResult
from dong_claude.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from dong_claude.core.config import DongConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: DongConfig) -> None:
    try:
...
async def _ping(config: DongConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{dong_claude.__version__}"},
    }

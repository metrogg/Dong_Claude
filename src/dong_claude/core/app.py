...
import dong_claude
from dong_claude.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from dong_claude.core.bus.envelope import EventPushEnvelope
from dong_claude.core.config import DongConfig, get_config
from dong_claude.core.events.bus import EventBus
from dong_claude.core.llm.provider import AnthropicProvider
from dong_claude.core.logging_setup import setup_logging
...
from dong_claude.core.permissions.storage import load_policy_file
from dong_claude.core.runner import AgentRunner
from dong_claude.core.runs import events_file, new_run_id
from dong_claude.core.session import SessionManager, SessionStore
from dong_claude.core.trace.record import TraceRecord
from dong_claude.core.trace.writer import TraceWriter
from dong_claude.core.transport.ipc_broadcaster import IpcEventBroadcaster
from dong_claude.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    def __init__(self) -> None:
        ...
        self._config: DongConfig | None = None
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=dong_claude.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in Path("~/.dong/sessions").expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.dong/policy.toml").expanduser()
        ...
        sessions_root = Path("~/.dong/sessions").expanduser()
        ...
        logger.info("dong-core %s listening addr=%s", dong_claude.__version__, addr)

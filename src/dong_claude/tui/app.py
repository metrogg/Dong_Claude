...
from dong_claude.core.skills.loader import SkillLoader
from dong_claude.core.transport.socket_client import IpcError, SocketClient
...
class DongTuiApp(App[None]):
    """DongClaude TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    _BANNER = (
        "[bold cyan]  DongClaude — Local Agent Runtime[/bold cyan]\n"
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]DongClaude[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]DongClaude[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]DongClaude[/bold]  [red]subscribe error: {e}[/red]")
            finally:
...
# TUI 入口：读取配置并启动 DongTuiApp
def run(config: DongConfig, replay_run_id: str | None = None) -> None:
    app = DongTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
